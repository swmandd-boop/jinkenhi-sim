...
/* 新機能A（§5）の表示条件を §6 に揃える：必要人件費率は nmin×atgt で計算するため、
   下回れない平均年収が未入力（初期値のまま）だと、法人が決めた線ではなく**入力例**を
   判定の根拠にしてしまう。よって atgt 未入力なら数値を出さず入力を促す。
   §5と§6で片方だけ出る状態を作らない。（初期値は検証の対象ではなく入力例＝分類①） */
...
test("UNIT-04 未入力状態の促し文でも金額の単位は「万円／年」", () => {
  for (const svc of ["tsuusho", "tokuyou", "unit", "roken"]) {
    ...
    for (const sel of ["#need-ratio", "#anchor"]) {
      const s = t.txt(sel);
      assert.ok(s.includes("初期値"), `${svc} ${sel}: 未入力時の促し文が出ていない → ${s.slice(0, 120)}`);
      const bad = s.match(/万円(?!／年)/g);
      assert.equal(bad, null, `${svc} ${sel}: 「万円」で終わる金額が ${bad && bad.length} 件ある → ${s}`);
    }
  }
});

test("PUB-05 既定値（法定福利費率）は未検証の明示と確かめ方を同じ項目に持つ", () => {
  /* ★.caveat 全体で探すと、別の項目にある同じ文で通ってしまう（実際に一度そうなった）。
     「未検証」を主張している項目そのものに確かめ方があることを見る。 */
  const t = open();
  const li = [...t.d.querySelectorAll(".caveat li")]
    .map(e => e.textContent.replace(/\s+/g, ""))
    .find(s => s.includes("法定福利費率16.5％は") && s.includes("サンプル値（未検証）"));
  assert.ok(li, `法定福利費率の「サンプル・未検証」の明示が消えている`);
  assert.ok(li.includes("決算書の「法定福利費」を「給与費」で割れば"),
    `未検証と書いている項目に確かめ方が併記されていない: ${li}`);
});

/* D の例外：単価は年額に揃えない。かわりにラベルで期間が分かること。 */
test("PUB-06 1人1日あたり収入は年額に揃えず、ラベルで期間が分かる", () => {
  const t = open();
  const row = [...t.d.querySelectorAll(".readout .row")]
    .find(e => e.textContent.includes("1人1日あたり収入"));
  assert.ok(row, `1人1日あたり収入の行が見つからない`);
  assert.ok(/1人1日あたり/.test(row.textContent), `ラベルに期間（1日あたり）が明記されていない`);
  assert.ok(!row.textContent.includes("万円／年"), `単価が年額に揃えられている（例外の扱いになっていない）`);
});
